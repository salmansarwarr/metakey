...
const USDT_AS_TOKEN = new Token(ChainId.BSC, USDT_AS_STRING, 18)
const CAKE_USDT_LP = '0x804678fa97d91B974ec2af3c843270886528a9E6'

describe('getLpAddress', () => {
  it('returns correct LP address, both tokens are strings', () => {
    expect(getLpAddress(CAKE_AS_STRING, USDT_AS_STRING)).toBe(CAKE_USDT_LP)
  })
  it('returns correct LP address, token1 is string, token 2 is Token', () => {
    expect(getLpAddress(CAKE_AS_STRING, USDT_AS_TOKEN)).toBe(CAKE_USDT_LP)
  })
  it('returns correct LP address, both tokens are Token', () => {
    expect(getLpAddress(CAKE_AS_TOKEN, USDT_AS_TOKEN)).toBe(CAKE_USDT_LP)
  })
