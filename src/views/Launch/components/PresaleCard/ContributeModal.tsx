...
const ContributeModal: React.FC<ContributeModalProps> = ({
  ...
  const lpTokensToStake = new BigNumber(val ? val : '0')
  ...
  return (
    <Modal title={t(`Contribute ${tokenName}`)} onDismiss={onDismiss}>
      ...
      />
      {
        lpTokensToStake.gt(0) && lpTokensToStake.lt(minContribution) && (
          <Text mt="10px" color="#cc0000">{t(`The minimum contribution is ${minContribution} BNB.`)}</Text>
        )
      }
      {referralState === 0 && 
      ...
      <ModalActions>
        ...
        <Button
          width="100%"
          disabled={
            pendingTx || !lpTokensToStake.isFinite() || lpTokensToStake.eq(0) || lpTokensToStake.gt(fullBalanceNumber) || lpTokensToStake.lt(minContribution)
          }
          onClick={async () => {
            setPendingTx(true)
            try {
