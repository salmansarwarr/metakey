...
describe('limitOrders/utils/getRatePercentageDifference', () => {
  describe('18 decimal tokens', () => {
    const marketPrice = new Price(CAKE, USDT, EIGHTEEN_DECIMALS, TEN) // 10 USDT per 1 CAKE
    it('returns correct positive percentage', () => {
      const price = new Price(CAKE, USDT, EIGHTEEN_DECIMALS, FIFTEEN) // 15 USDT per 1 CAKE
    ...
    it('returns correct negative percentage', () => {
      const price = new Price(CAKE, USDT, EIGHTEEN_DECIMALS, FIVE) // 5 USDT per 1 CAKE
      const rate = getRatePercentageDifference(marketPrice, price)
      const expectedRate = new Percent(-50, 100)
      expect(expectedRate.equalTo(rate)).toBe(true)
    })
    it('returns correct equal percentage', () => {
      const price = new Price(CAKE, USDT, EIGHTEEN_DECIMALS, TEN) // 50 USDT per 1 CAKE
      const rate = getRatePercentageDifference(marketPrice, price)
  ...
  describe('18 decimal token and 8 decimal token', () => {
    const marketPrice = new Price(DOGE, USDT, EIGHTEEN_DECIMALS, TEN) // 10 USDT per 1 DOGE
    it('returns correct positive percentage', () => {
      const price = new Price(DOGE, USDT, EIGHTEEN_DECIMALS, FIFTEEN) // 15 USDT per 1 DOGE
      const rate = getRatePercentageDifference(marketPrice, price)
      const expectedRate = new Percent(50, 100)
      expect(expectedRate.equalTo(rate)).toBe(true)
    })
    it('returns correct negative percentage', () => {
      const price = new Price(DOGE, USDT, EIGHTEEN_DECIMALS, FIVE) // 5 USDT per 1 DOGE
      const rate = getRatePercentageDifference(marketPrice, price)
      const expectedRate = new Percent(-50, 100)
      expect(expectedRate.equalTo(rate)).toBe(true)
    })
    it('returns correct equal percentage', () => {
      const price = new Price(DOGE, USDT, EIGHTEEN_DECIMALS, TEN) // 50 USDT per 1 DOGE
      const rate = getRatePercentageDifference(marketPrice, price)
      const expectedRate = new Percent(0, 100)
      expect(expectedRate.equalTo(rate)).toBe(true)
    })
